const amenities = [
  {
    ...
    icon: (
      <svg className="w-9 h-9 text-teal-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.2}>
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="space-y-4">
          {amenities.map((amenity) => (
            <div
              key={amenity.title}
              className="flex flex-col sm:flex-row items-center sm:items-start gap-4 md:gap-6 p-5 md:p-8 rounded-2xl border border-slate-100 hover:border-teal-200 hover:shadow-md transition-all bg-white group"
            >
              <div className="flex-shrink-0 w-14 h-14 md:w-16 md:h-16 rounded-xl bg-teal-50 group-hover:bg-teal-100 transition-colors flex items-center justify-center">
                {amenity.icon}
              </div>
              <div className="text-center sm:text-left pt-0 sm:pt-1">
                <h3 className="font-semibold text-slate-900 text-base md:text-lg">{amenity.title}</h3>
